...
Partial Class Com_Report_Survey
    ...
    Protected Sub gvInbox_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvInbox.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Count += 1
            Dim franid As Integer = CInt(DataBinder.Eval(e.Row.DataItem, "FranchiseID"))
            e.Row.Attributes.Add("OnClick", "javascript:window.location='ActivityDetail.aspx?ID=" & CType(e.Row.FindControl("lblID"), Label).Text & "&PrevPage=3&FranchiseID=" & franid & "&ClientID=" & hdnClientID.Value & "';return false;")
        End If
