<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DMenuLicenseManagement.aspx.vb" Inherits="DMenu_LicenseManagement" Theme="Default" %>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="height:100%;">
  <tr class="GalleryTitle" style="background-image:url('../images/bg_mainHdr_1.jpg');height:31px;">
    <td>&nbsp;<span class="IdeaTitle">DMenu Licenses - 
        ...
        <asp:GridView ID="gvwKeys" runat="server" AutoGenerateColumns="False" Width="100%"  SkinID="gvwSilver"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="Store Nick" SortExpression="StoreNick" >
                    <ItemTemplate>
                       <span title='<%# Eval("StoreNick") %>'><%#Functions.StringTruncate(Eval("StoreNick"), 15)%></span>
                    </ItemTemplate>
                    <FooterTemplate>
                        &nbsp;
                    </FooterTemplate>
                ...
                <asp:TemplateField HeaderText="Displays" SortExpression="NumberOfDisplays">
                    <ItemTemplate>
                        <div align="right"><%#Eval("NumberOfDisplays")%>&nbsp;</div>                        
                    </ItemTemplate>
                    <FooterTemplate>
                        &nbsp;
                    </FooterTemplate>
...
<asp:ObjectDataSource ID="odsSBSLicenses" runat="server" TypeName="Portal.BLL.LM.DMenuLicenseManagement" 
     SelectMethod="GetAllDMenuLicenses" MaximumRowsParameterName="maximumRows" StartRowIndexParameterName="startRowIndex" 
     SelectCountMethod="GetAllDMenuLicensesCount" SortParameterName="sortBy" EnablePaging="true" EnableViewState="true">
    <SelectParameters>
...
<asp:SqlDataSource ID="sdsStates" runat="server" ConnectionString="<%$ ConnectionStrings:PortalConnectionString %>"
    ...
    <script language="javascript" type="text/javascript">
        ...
        function ShowNewLicenseDiv(){            
            ...
            ifNewLicense.location='DMenuNewLicense.aspx';
        ...
        function ShowDetail(licenseId){
          ...
          window.location='DMenuLicenseDetail.aspx?LicenseID='+licenseId+'&ddlClient='+ddlClient+'&ddlStates='+ddlStates+'&ddlFilters='+ddlFilters;
        }
         function ShowDetailReadonly(licenseId){
          var ddlClient = document.getElementById('<%= ddlClient.ClientID %>').value; 
          var ddlStates =  document.getElementById('<%= ddlStates.ClientID %>').value; 
          var ddlFilters =  document.getElementById('<%= ddlFilters.ClientID %>').value; 
          window.location='DMenuLicenseDetail.aspx?LicenseID='+licenseId+'&ddlClient='+ddlClient+'&ddlStates='+ddlStates+'&ddlFilters='+ddlFilters+'&PrevPage=MyNetsolace';
        }
        function NavigateToPrintVersion()
        {
            ...
            URL = "DMenuLicensesReport.aspx" + QueryStringValues;
            window.open(URL);
        }
        function OnXLSDownload()
        {
            ...
            QueryStringValues += "&SortBy=" + document.getElementById('<%=hdnSortBy.ClientID %>').value;  
            QueryStringValues += "&State=" + ddlStates.options[ddlStates.selectedIndex].text;  
            URL = "DMenuLicensesReport.aspx" + QueryStringValues+'&IsXLS=True'; 
