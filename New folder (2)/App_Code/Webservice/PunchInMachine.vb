...
Public Class PunchInMachine
    ...
    Public Function GetLocalTime() As String
        ...
        For i As Integer = 0 To strIPArray.Length - 1
            If strIPArray(i).Trim() = HttpContext.Current.Request.UserHostAddress.ToString().Trim() Then
                LocalTime = Functions.GetTimeByZone(ConfigurationManager.AppSettings("NetsolaeIsbTimeOffset"), Now)
            End If
        Next
    ...
    <WebMethod()> _
    Public Function GetThemes(ByVal LastSyncDate As DateTime, ByVal OfficeID As Integer) As Byte()
        If ValidateIP() Then
            Dim db As New DBAccess()
            Dim dsThemes As New DataSet()
            db.AddParameter("@LastSyncDate", IIf(LastSyncDate = DateTime.MinValue, DBNull.Value, LastSyncDate))
            db.AddParameter("@ClientID", OfficeID)
            dsThemes = db.ExecuteDataSet("TimeMachine_Theme_GetForTimeMachine")
            db.Dispose()
            Return Functions.CompressDataSet(dsThemes)
        Else
            Return Nothing
        End If
    End Function
    <WebMethod()> _
    Public Function GetImagesWithType(ByVal LastSyncDate As DateTime, ByVal OfficeID As Integer) As Byte()
        If ValidateIP() Then
            Dim db As New DBAccess()
            Dim dsImages As New DataSet()
            db.AddParameter("@LastSyncDate", IIf(LastSyncDate = DateTime.MinValue, DBNull.Value, LastSyncDate))
            db.AddParameter("@ClientID", OfficeID)
            dsImages = db.ExecuteDataSet("TimeMachine_Image_GetForTimeMachineWithType")
            db.Dispose()
            Return Functions.CompressDataSet(dsImages)
        Else
            Return Nothing
        End If
    End Function
