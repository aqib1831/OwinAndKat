...
<body  style ="margin :0px;background-color:#E4F1F7;" >
	...
	<table width="100%" style="height:100%; margin:0px; padding:0px;background-color:#E4F1F7; " cellspacing="0" cellpadding = "0" border="0">
			 <tr class="UnderlinedRow">
				 <td  style="height:20px;background-color:#A0CDEE;font-size:12px;font-weight:bold;width:50%;">
					<asp:UpdatePanel ID="UpdatePanel1" runat="server" UpdateMode="Conditional" RenderMode="Inline">
						<ContentTemplate>
							<pre style="height:20px;font-family:Arial;font-size:12px;font-weight:bold;padding:3px;float:left;">Recent Tickets  |  <asp:Label runat="server" ID="lblErrorCount" />&nbsp;(<asp:Label runat="server" ID="lblFilter" Text="all" class="NonBold" />)</pre>
						</ContentTemplate>
						<Triggers >
				 ...
				 <td style="background-color:#A0CDEE;padding:3px;text-align:right;">
					 <asp:LinkButton ID="lnkResetFilters" runat="server" CssClass="HomeButton">
						<div style="width:110px;" class="LeadDetailButton" onClick="javascript:resetFilters();">
							Reset Filters 
							<img runat="server" src="~/images/blt_resetFilters.gif" align="absmiddle" width="10" height="10" style="border:0px;"/>
						</div>
					 </asp:LinkButton>
				 </td>
			 </tr>
					...
					<tr>
					  <td style="text-align:left; width: 100px; padding-left:3px; padding-right:3px; white-space:nowrap; padding-top:10px;">
						  <div class="LeadDetailButton" onClick="javascript:StateSaveNewTicket();"
							  style="width: 100px">
								New Ticket <img align="absMiddle"  height="10" src="~/images/blt_resetFilters.gif" runat="server" width="10" />
						  </div>
					  </td>
					  <td style="width:10px;text-align:center;">
						 <img runat="server" src="~/images/pixel_darkBlue.jpg" width="1" height="30" align="absmiddle" />
					  </td>
					...
					<div style="overflow:auto;height:100%;background-color:#E4F1F7;border:1px solid #ACBDD5;" id="_gridViewPanel1"  class="ScrollBarHomeTicket">
					  <asp:UpdatePanel ID="UpdatePanel2"
						...
						<asp:GridView ID="gvReleases" runat="server" AllowSorting="True" AutoGenerateColumns="False"
							DataKeyNames="ID,ReleaseID" DataSourceID="ObjectDataSource1" EnableTheming="True" SkinID="GridViewNormal" OnRowCreated="gvReleases_RowCreated" OnRowDataBound="gvReleases_RowDataBound" EnableViewState="false">
								  ...
								  <div style="width:100%; border:0px black solid;" >
									  <table width="100%" border="0" cellpadding="0" cellspacing="0"  style="height:20px;">
										   <tr style="text-align:left;">
												 ...
												 <td style="width:10px; " align="right" ><img alt="" runat="server" src="~/images/spacer.gif" width="15" height="1" align="absmiddle" /></td>
												 <td style="width:1px; " align="center"><img alt="" runat="server" src="~/images/pixel_darkBlue.jpg" width="1" height="20" align="absmiddle" /></td>
												 <td style="width:10px;" align="right"><img alt="" runat="server" src="~/images/spacer.gif" width="15" height="1" align="absmiddle" /></td>
							...
							<Columns>
								<asp:TemplateField HeaderText="^"  HeaderStyle-HorizontalAlign="Center"  HeaderStyle-Font-Bold="true" SortExpression="Priority" HeaderStyle-Wrap="false" ItemStyle-Wrap="false" >
									<ItemTemplate>
										<img  src="../../images/pm_flag.gif"  style='<%#  IIf(Functions.IFNULL(Eval("Status"), "")  = "Confirm Solved" Or Functions.IFNULL(Eval("Status"), "")  = "Feedback Requested", "display:inline", "display:none") %>'  />
										<img src='<%= ConfigurationManager.AppSettings("ImagesURL") %><%#Eval("Priority") %>'  />
									</ItemTemplate>
									<ItemStyle Width="2%" />
								</asp:TemplateField> 
								<asp:TemplateField HeaderText="Ref #" SortExpression="RefNo">
									<ItemTemplate><span><%#Eval("RefNo")%></span></ItemTemplate>
								 ...
								 <asp:TemplateField HeaderText="Name" SortExpression="Name" HeaderStyle-Wrap="false" >
									<ItemTemplate>
										<span title='<%#Server.HtmlEncode(Eval("Name"))%>'><%#Functions.StringTruncate(Server.HtmlEncode(Functions.IfNull(Eval("Name"), "")), 30)%></span>
									</ItemTemplate>
									 <ItemStyle Width="25%" Wrap="false"  />
								</asp:TemplateField>
								 <asp:TemplateField HeaderText="Status" SortExpression="Status" HeaderStyle-Wrap="false">
									<ItemTemplate>
										<span title='<%#Server.HtmlEncode(Functions.IfNull(Eval("Status"),""))%>'><%#Functions.StringTruncate(Server.HtmlEncode(Functions.IfNull(Eval("Status"), "")), 25)%></span>
									</ItemTemplate>
