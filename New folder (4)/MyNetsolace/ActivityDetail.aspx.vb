...
Partial Class MyNetsolace_ActivityDetail
    ...
    Protected Sub btnYes_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnYes.Click
        ...
        With objActivityNotes
            ...
            If rdoGreate.Checked Then
                message.Append("<img src='https://qa.netsolace.com/Portal/images/great-small.jpg' title='Great'/>" & vbCrLf & "<span style='padding-top:2px;'>It was Great</span>" & vbCrLf)
            ElseIf rdoOk.Checked Then
                message.Append("<img src='https://qa.netsolace.com/Portal/images/ok-small.jpg' title='OK'/>" & vbCrLf & "<span style='padding-top:2px;'>It was OK</span>" & vbCrLf)
            ElseIf rdoUnHappy.Checked Then
                message.Append("<img src='https://qa.netsolace.com/Portal/images/happy-small.jpg' title='Not Happy'/>" & vbCrLf & "<span style='padding-top:2px;'>I am not happy</span>" & vbCrLf)
            End If
            If Not String.IsNullOrEmpty(txtFeedBack.Text.Trim) Then
                message = message.Append(vbCrLf & txtFeedBack.Text.Trim & vbCrLf & vbCrLf)
            End If
            .Notes = message.ToString() & "Ticket has been closed."
            message = Nothing
            'Specifying whether the notes are from franchisee or franchisor
            If Not Session("FranchiseContactID") Is Nothing Then
                .PlacedBy = Enumeration.PlacedBy.Franchisee
            ...
            .NotesDate = DateTime.Now
            intNewNotesID = .AddNotes()
            If rdoUnHappy.Checked Then
                SendUnHappySurveyEmail(.Notes)
            End If
        End With

        Dim objActivitySurvey As New Portal.BLL.ActivitySurvey
        With objActivitySurvey
            .ActivityID = CInt(hdnID.Value)
            If rdoGreate.Checked Then
                .ActivityRate = Enumeration.ActivitySurveyRate.Great
            ElseIf rdoOk.Checked Then
                .ActivityRate = Enumeration.ActivitySurveyRate.OK
            ElseIf rdoUnHappy.Checked Then
                .ActivityRate = Enumeration.ActivitySurveyRate.UnHappy
            End If

            If Not Session("FranchiseContactID") Is Nothing Then
                .EnteredByClientContactID = 0
                .EnteredByFranContactID = CInt(Session("FranchiseContactID"))
            ElseIf Not Session("MyNetsolace_ClientContactID") Is Nothing Then
                .EnteredByFranContactID = 0
                .EnteredByClientContactID = CInt(Session("MyNetsolace_ClientContactID"))
            End If
            If Not String.IsNullOrEmpty(txtFeedBack.Text.Trim) Then
                .SurveyComments = txtFeedBack.Text.Trim
            Else
                .SurveyComments = String.Empty
            End If
            Dim Affected As Integer = .Add()
        End With
        With ObjectActivity
            'Close the Ticket 
            .ID = CInt(hdnID.Value)
        ...
        End With
    End Sub
    Public Sub SendUnHappySurveyEmail(ByVal Notes As String)
        Dim StoreNumber As String = Portal.BLL.Franchises.GetStoreNickByFransupportFranchiseID(CInt(Session("FranchiseID")), CInt(Session("MyNetsolace_ClientID")))
        Dim mail As New Email
        mail.MailTo = "support@netsolace.com"
	mail.MailFrom = "do-not-reply@netsolace.com"
        mail.DisplayName = "Netsolace Support"
        mail.Subject = "Ticket number " & CType(hdnID.Value, Integer) & " - Store number " & StoreNumber & " - NOT HAPPY" 
        Dim body As String = String.Empty
        body = My.Computer.FileSystem.ReadAllText(HttpContext.Current.Server.MapPath("~/templates/EmailUnHappySurvey.htm"))
        body = body.Replace("***EmailDetail***", Notes.Replace(vbCrLf, "<br>"))
        mail.Detail = body
        mail.Send_mail()
        mail = Nothing
    End Sub
