...
Partial Class PM_TicketsDetail
    ...
    Dim LinkTickettable As DataTable
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'If ViewState("LinkTicket") IsNot Nothing Then
        '    LinkTickettable = CType(ViewState("LinkTicket"), DataTable)
        'End If
        Dim s As String = hdnID.Value
        LinkTickettable = Portal.BLL.PM.PM_Ticket_Features.PM_TicketFeature_TicketLink_GetByTicketID(HttpContext.Current.Request.QueryString("ID").ToString())
        If Not Page.IsPostBack Then
            ...
            If String.IsNullOrEmpty(Request.QueryString("ID")) = False AndAlso IsNumeric(Request.QueryString("ID")) Then
                ...
                hdnGroupID.Value = CType(Enumeration.ResourceType.ProjectManagerLead, Integer)
                ...
                If String.IsNullOrEmpty(Request.QueryString("PrevPage")) = False AndAlso IsNumeric(Request.QueryString("PrevPage")) Then
                    ...
                    Select Case PrevPage.Value
                        ...
                        Case 2 'Feature
                            btnBack.Attributes.Add("OnClick", "window.location='" & ResolveClientUrl("~/MyNetsolaceClient/PM/Features.aspx?ID=" & hdnReleaseID.Value) & "&ddlClients=" & Session("MyNetsolace_ClientID") & "&ddlCategories=" & ddlCategories & "&ddlTypes=" & ddlTypes & "&ddlFilter=" & ddlFilter & "&txtSearch=" & txtSearch & "&reClients=" & reClient & "&reCategories=" & reCatagory & "'; return false; ")
                        Case 3 'Tickets
                            btnBack.Attributes.Add("OnClick", "window.location='" & ResolveClientUrl("~/MyNetsolaceClient/PM/Tickets.aspx?ddlClients=" & ddlClients) & "&ddlCategories=" & ddlCategories & "&ddlTypes=" & ddlTypes & "&ddlFilter=" & ddlFilter & "&txtSearch=" & txtSearch & "'; return false; ")
                        ...
                        Case 4 'MyTickets
                            btnBack.Attributes.Add("OnClick", "window.location='" & ResolveClientUrl("~/MyNetsolaceClient/PM/MyTickets.aspx?ddlClients=" & ddlClients) & "&ddlCategories=" & ddlCategories & "&ddlFilter=" & ddlFilter & "&txtSearch=" & txtSearch & "'; return false; ")
                        ...
                        Case 5 'Tickets History
                            btnBack.Attributes.Add("OnClick", "window.location='" & ResolveClientUrl("~/MyNetsolaceClient/PM/TicketsHistory.aspx?ddlClients=" & ddlClients) & "&ddlCategories=" & ddlCategories & "&ddlTypes=" & ddlTypes & "&ddlFilter=" & ddlFilter & "&txtSearch=" & txtSearch & "'; return false; ")
                        Case Else
        ...
        End If
        If LinkTickettable.Rows.Count > 0 Then
            linkedTickets.Visible = True
            linkedTickets.DataSource = LinkTickettable
            linkedTickets.DataBind()
        Else
            linkedTickets.Visible = False
        End If
                ...
                If hdnPreValueStatus.Value <> 5 AndAlso ddlActivityStatus.SelectedValue = 5 Then
                    With ObjNotes
                        ...
                        If .Notes_Add() > 0 Then
                            ...
                            rptrNotes.DataBind()
                        End If
                        ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "ShowSurveyForm", "ShowSurveyForm();", True)
                    End With
    ...
    Public Sub BindControl()
        ...
        If colTicket_Features.Count > 0 Then
            ...
            If colTicket_Features.Item(0).SubCatagoryName = String.Empty Then
            ...
            If colTicket_Features.Item(0).TicketOwnerName <> String.Empty Then
                tdproductowner.Visible = True
            Else
                tdAreaDeveloper2.Visible = True
            End If
            ...
            If Not Page.IsPostBack Then
                ...
                If colTicket_Features.Item(0).CorporateName = "" Then
                    ...
                    lblAreaDeveloper.Text = ""
                Else
                    tdAreaDeveloper.Visible = True
                    lblAreaDeveloper.Text = colTicket_Features.Item(0).CorporateName
                End If
                ...
                End If
                'If colTicket_Features.Item(0).WaitingForUserID > 0 Then
                '    Dim item As ListItem = ddlAssignedto.Items.FindByValue(colTicket_Features.Item(0).WaitingForUserID.ToString())
                '    If item IsNot Nothing Then
                '        ddlAssignedto.SelectedValue = colTicket_Features.Item(0).WaitingForUserID
                '    End If
                'End If
            ...
            End If
            ' mtahir 
            'If colTicket_Features.Item(0).IsFeedbackRequired = True Then
            '    ddlActivityStatus.SelectedValue = 8
            'End If

            If colTicket_Features.Item(0).IsFeedbackRequired = True Then
                ddlActivityStatus.SelectedValue = 8
                dvIsfeedbackRequired.Visible = True
            Else
                dvIsfeedbackRequired.Visible = False
            End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Dim Result As Boolean
        dvIsfeedbackRequired.Visible = False               ' To Hide that alert of attention
        ...
        If (Session("update").ToString() = ViewState("update").ToString()) Then
            ...
            With ObjNotes
                ...
                .SubmittedBy = HttpContext.Current.Session("AdminID")
                .ClientContactID = Session("MyNetsolace_ClientContactID")
                .DateEntered = DateTime.Now
                .IsByAdmin = 1
                ...
                Result = PM_Ticket_Features.UpdateAssingTo(CType(hdnID.Value, Integer), ddlAssignedto.SelectedItem.Value)
            ...
            For j = 0 To ClientContactId.Length - 1
                For i = 0 To dtUsers.Rows.Count - 1
                    If dtUsers.Rows(i)("ID") = ClientContactId(j) Then
                        PM_Ticket_Features.SendMailToClientContacts(dtUsers.Rows(i)("Name"), dtUsers.Rows(i)("Email"), ClientContactEmail, ClientContactName, "", subject, strDescription, hdnRefNo.Value, Me.lblActivitySubject.Text.Trim(), strStatus, IIf(hdnIsDueDateShowToClient.Value = True, CDate(hdnDueDate.Value).Date, "To Be Determined"), strPriority, Me.lblActivityType.Text, Me.lblClientName.Text, Me.lblCategoryName.Text, False, IIf(dtUsers.Rows(i)("AreaDeveloperId") <> 0, "<a href=""" & dtUsers.Rows(i)("Link") & """>" & dtUsers.Rows(i)("Link") & "</a>", ""), IIf(dtUsers.Rows(i)("AreaDeveloperId") <> 0, dtUsers.Rows(i)("Link"), ""), IIf(dtUsers.Rows(i)("AreaDeveloperId") <> 0, True, False))
                    End If
                Next
            Next
        ...
        Else
            ...
            For i = 0 To dtUsers.Rows.Count - 1
                ...
                ElseIf dtUsers.Rows(i)("Type") = Enumeration.ContactsType.Netsolace Then
                    PM_Ticket_Features.SendMailToClientContacts(dtUsers.Rows(i)("Name"), dtUsers.Rows(i)("Email"), ClientContactEmail, ClientContactName, "", subject, strDescription, hdnRefNo.Value, Me.lblActivitySubject.Text.Trim(), strStatus, CDate(hdnDueDate.Value).Date, strPriority, Me.lblActivityType.Text, Me.lblClientName.Text, Me.lblCategoryName.Text, True)
    ...
    Private Function CheckValidityEmail(ByVal strEmail As String) As Boolean
        Dim ClientContactEmail As String = Session("MyNetsolace_ClientContactEmail")
        If strEmail <> Nothing AndAlso Functions.IsValidEmail(strEmail) AndAlso CStr(ClientContactEmail) <> Nothing AndAlso Functions.IsValidEmail(ClientContactEmail) Then
            Return True
        End If
        Return False
    End Function
            ...
            For i = 0 To rptClientContacts.Items.Count - 1
                ...
                If chkBox.Checked = True Then
                    ...
                    If strAfterClientContactID.ToString = "" Then
                        strAfterClientContactID.Append(j & ". " & chkBox.Attributes("ToolTip") + vbCrLf+"</br>")
                    Else
                        strAfterClientContactID.Append(j & ". " & chkBox.Attributes("ToolTip") + vbCrLf +"</br>")
                    End If
    ...
    Protected Sub rptClientContacts_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptClientContacts.ItemDataBound
        ...
        End If
    End Sub
    Protected Sub ddlAssignedto_DataBound(sender As Object, e As EventArgs) Handles ddlAssignedto.DataBound
        If colTicket_Features.Item(0).WaitingForUserID > 0 Then
            Dim item As ListItem = ddlAssignedto.Items.FindByValue(colTicket_Features.Item(0).WaitingForUserID.ToString())
            If item IsNot Nothing Then
                ddlAssignedto.SelectedValue = colTicket_Features.Item(0).WaitingForUserID
